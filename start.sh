...
export API_LISTEN_URL VITE_API_URL API_HOST API_PORT WEB_HOST WEB_PORT
...
echo "🌐 Web dev: http://${WEB_HOST}:${WEB_PORT} (VITE_API_URL=${VITE_API_URL})"
npm run dev --prefix max-web -- --host "${WEB_HOST}" --port "${WEB_PORT}" & WEB_PID=$!
